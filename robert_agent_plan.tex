...
\begin{itemize}
    ...
    \item explain them in chemistry-friendly language,
    \item suggest one to three next actions.
\end{itemize}

The LLM should \textbf{not} be responsible for discovering all reasoning from scratch.

\subsubsection*{Step 6.2 --- Create a minimal prompt}
A minimal prompt can say:

\begin{quote}
You are assisting a chemist who ran ROBERT. Use only the structured context provided. Explain why the user obtained this ROBERT score, identify the most likely limiting factors, and recommend up to three concrete next steps. Do not invent file contents or descriptor meanings that are not present in the provided context.
\end{quote}

\subsubsection*{Step 6.3 --- Save explanations as text}
